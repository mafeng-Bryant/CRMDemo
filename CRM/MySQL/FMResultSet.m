...
- (DetailProduct*)getMyProduct
{
        
//     [sqlstr appendFormat:@"insert into %@(detailProductId,clickOut,fCreateTime,detailProductTitle,priceNumber,fUnit,FStoreAvatarImageId) values('%@','%@','%@','%@','%@','%@','%@')",tablename,detailProductId,clickOut,fCreateTime,detailProductTitle,priceNumber,fUnit,FStoreAvatarImageId];
    ...
    if (num_cols > 0) {
        ...
        for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
            ...
            if (columnIdx ==0) {
                product.detailProductId = objectValue;
                
                
            }else if (columnIdx ==1){
                
                product.clickCount = [objectValue integerValue];
   
                
            }else if (columnIdx ==2){
                
                product.fCreateTime = objectValue;
                
                
            }else if (columnIdx ==3){
                product.detailProductTitle = objectValue;
                
                
            }else if (columnIdx ==4){
                
                product.priceNumber = objectValue;
                
            }
            else if (columnIdx ==5){
                
                product.fUnit = objectValue;
                
            }  else if (columnIdx ==6){
                
                product.FStoreAvatarImageId = objectValue;
                
            } else if (columnIdx ==7){
                
                product.FMasterCatalogId = objectValue;
...
- (MFMessage*)getAwayMessage
{
   // MFMessage* message = [[MFMessage alloc]init];

// [sqlstr appendFormat:@"insert into %@(LOGINUSER,TALKUSER,MSG,messageType,messMediaType) values('%@','%@','%@','%@','%@')",tableName,loginId,talkId,msg,messageType,messageMediaType];
    NSUInteger num_cols = (NSUInteger)sqlite3_data_count([_statement statement]);
    ...
    if (num_cols > 0) {
        ...
        for (columnIdx = 0; columnIdx < columnCount; columnIdx++) {
            ...
            if (columnIdx ==0) {
             //   message.tojid = objectValue;

                
            }else if (columnIdx ==1){
   
             //   message.fromjid =objectValue;
                
            
            }else if (columnIdx ==2){
  
             //   message.text =objectValue;
                
            
            }else if (columnIdx ==3){
              //  message.messageRecivingType =[objectValue intValue];
                
            
            }else if (columnIdx ==4){
            
              //  message.messageMediaType =[objectValue intValue];

            }
            else if (columnIdx ==5){
                
           //  message.messageSendDate =objectValue;
                
            }  else if (columnIdx ==6){
                
             //   message.voiceDuration =objectValue;
                
            }
            else if (columnIdx ==7){
                
            //    message.locationDescration =objectValue;
